...
import numpy as np
from utils_koi import UtilKoi
from pdb import set_trace
from convert_csv import csv_from_excel


class WorkflowJapanKOI:
    def __init__(self):
        self.query = Query()
        ...
        self.country_prd = "Fem"

    def process_data_frame(self, xlsx, df, agg_type):
        Utils = UtilKoi()
        try:
            df, country, ref_prd = Utils.process_data(
                xlsx, df, agg_type, self.country_prd)
            # set_trace()
    ...
    def run(self, xlsx, sheet_name, agg_type, csv_name):
        try:
            # df = pd.read_excel(
            #     "{0}".format(xlsx), sheet_name=sheet_name, header=None)
            csv_from_excel(xlsx, sheet_name, csv_name)
            df = pd.read_csv(csv_name, sep=',', error_bad_lines=False,
                             index_col=False, dtype='unicode')
            df = self.process_data_frame(xlsx, df, agg_type)
        except Exception as err:
            print(err)
